...
update_status ModuleGame::Update()
{
	if (current_state == START_MENU) {
		...
		return UPDATE_CONTINUE; // No ejecutamos l骻ica de juego
	}

	// 2. 倒计时状态
	if (current_state == COUNTDOWN) {
		countdown_timer -= GetFrameTime(); // 减去上一帧的时间
...
update_status ModuleGame::PostUpdate()
{
	...
	if (current_state == START_MENU) {
		...
		// Volvemos a activar la c醡ara por si el motor la necesita para el siguiente m骴ulo
	...
	if (current_state == COUNTDOWN) {
		...
		// 不需要在这里 BeginMode2D，因为这是 PostUpdate 的末尾
		return UPDATE_CONTINUE;
